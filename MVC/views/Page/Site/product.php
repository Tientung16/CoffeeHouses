  ...
  ?>
    ...
    <div class="card mt-1">
        ...
        <div class="product-content">
          ...
          ?>
                    <p class="new-price price-bg <?= $item['size'] ?>" hidden name="<?= $item['price']*(100-$row['price_sale'])/100 ?>"
                        style="font-size: 3rem;"><?= number_format($item['price'], 0, ",", ",") ?> VNĐ</p>
                    <?php
          }
          ?>
                    <p class="new-price price-bg" id="priceSize" name="<?= $row['size'] ?>" style="font-size: 3rem;">
                        <?= number_format($row['price']*(100-$row['price_sale'])/100, 0, ",", ",") ?> VNĐ</p>
                    <p class="new-price slogan" style="font-size: 16px;">Ở đâu rẻ hơn, Coffee House hoàn tiền</p>
                </div>
                <div class="product-detail">
                    <ul class="product-detail__list">
                        <li class="product-detail__item">Danh mục: <span><?= $row['category_name'] ?></span></li>
                        <li class="product-detail__item">Số lượng: <span><?= ($row['amount']==0)?"Hết hàng":$row['amount'] ?></span></li>
        ...
        <button name="sizeL" class="size">L</button> -->
                    ...
                    <input class="sizes" hidden name="size" type="radio" value="Nhỏ" id="sizeNho" checked>
                ...
                <div class="purchase-info">
                    ...
                    <input type="number" name="num" min="1" max="<?= $row['amount'] ?>" value="1" id="input">
                ...
                <input type="text" hidden id="pricePost2" name="price" value="<?= $row['price']*(100-$row['price_sale'])/100 ?>">
                <input type="text" hidden name="thumbnail" value="<?= $row['thumbnail'] ?>">
                <input type="text" hidden name="name" value="<?= $row['product_name'] ?>">

                <div class="click">
                    <button type="submit" name="addToCart" id="btn1" onclick=OutOfProduct()><i class="fas fa-shopping-cart"></i> Thêm vào giỏ
                        hàng</button>
                </div>
            </form>
        </div>
